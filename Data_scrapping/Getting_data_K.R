...
for (i in 1:length(urls)){
  ...
  AdjEM<- as.numeric(html_text(AdjEM))
  
  AdjO<- html_nodes(web, '.td-left:nth-child(6)')
  AdjO<- as.numeric(html_text(AdjO))
  
  AdjD<- html_nodes(web, '.td-left:nth-child(8)')
  AdjD<- as.numeric(html_text(AdjD))
  
  AdjT<- html_nodes(web, '.td-left:nth-child(10)')
  AdjT<- as.numeric(html_text(AdjT))
  
  luck<- html_nodes(web, '.td-left:nth-child(12)')
  luck<- as.numeric(html_text(luck))
  
  data <- data.frame('team' = team, 'AdjEM' = AdjEM, 'AdjO' = AdjO, 
